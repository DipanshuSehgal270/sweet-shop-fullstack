...
import { getAllSweets, getTrendingSweets, searchSweets, deleteSweet } from '../services/sweetService';
...
const DashboardPage = () => {
  ...
  const [trending, setTrending] = useState([]); // Separate state for trending
  ...
  const loadData = async () => {
    try {
      // Fetch both All Sweets and Trending in parallel for speed
      const [allData, trendingData] = await Promise.all([
        getAllSweets(),
        getTrendingSweets()
      ]);
      
      if (Array.isArray(allData)) setSweets(allData);
      ...
      <div className="container pb-5">
        ...
        <div className="mb-5">
            ...
            <div className="row g-4">
                ...
                {/* 1. TOP 2 BEST SELLERS (Now fetched directly from Backend) */}
                {trending.map(sweet => (
        ...
        <div className="d-flex justify-content-between align-items-center mb-4 mt-5 border-bottom pb-3">
            ...
            <div className="d-flex gap-2">
                <form onSubmit={handleSearch} className="d-flex gap-2">
                    ...
                    {searchTerm && (
                        <button type="button" className="btn btn-outline-secondary" onClick={() => {setSearchTerm(''); loadData()}}>✕</button>
                    )}
                </form>
